...
**3. Subject vs body 匹配错位**（commit st1 之后才暴露）：第一版 smoke-test 用 `git log --format=%H%x09%s`（`%s` = subject only），但 `session: <slug>` 标记按本项目惯例写在 commit body（subject 留给可读摘要）。第一次真跑后立刻被自己的 smoke gate FAIL。修复：换 `--format=%H%x09%B` + `git log -z`（null 分隔多 commit），匹配也改为在 fullMsg 全文 search。这恰好佐证了"AI 自己写的工具不实跑就有问题"——光看代码 review 看不出 subject vs body 的语义差。
